...
const searcherApplications = async (
  req: NextApiRequest,
  res: NextApiResponse,
) => {
  ...
  if (!isSearcher) {
    return res
      .status(500)
      .json({ ok: false, data: { message: "is not a searcher" } });
  }
  let applicants: Applicant[] = [];
  const assignmentIds = await allApplicationsForSearcher({ email });
  const retrievedAssignmentRecordPromises = assignmentIds.map((id) =>
    retrieveRecord(id),
  );
  const retrievedAssignment = await Promise.all(
    retrievedAssignmentRecordPromises,
  );

  applicants = uniqBy(
    retrievedAssignment.map((applicant) => {
      return {
        id: applicant.fields[
          ASSIGNMENTS_TABLE.columns.applicantRecordId
        ]?.toString(),
        name: applicant.fields[
          ASSIGNMENTS_TABLE.columns.applicantName
        ]?.toString(),
        searcherDecision:
          applicant.fields[ASSIGNMENTS_TABLE.columns.decision]?.toString(),
      };
    }),
    "id",
  );
