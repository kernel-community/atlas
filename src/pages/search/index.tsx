...
import {
  ...
  DECISIONS,
  DecisionToString,
} from "src/hooks/useApplicationDecision";
        ...
        <div className="form flex flex-col gap-3">
          <div className="flex flex-col gap-3 items-center">
            <div className="form-control">
              <label
                className="label cursor-pointer gap-3"
                onClick={() => setSelectedDecision(DECISIONS.yes.value)}
              >
                <input
                  type="checkbox"
                  name="checkbox"
                  className="checkbox checkbox-primary h-12 w-12"
                  checked={selectedDecision === DECISIONS.yes.value}
                />
            ...
            <RetroButton
              ...
              isLoading={
                selectedDecision === DECISIONS.yes.value && isSubmitting
              }
            >
              Submit
            </RetroButton>
          ...
          <div className="form-control mt-4 flex flex-row justify-end items-center">
            <label
              className="label cursor-pointer  gap-3"
              onClick={() => setSelectedDecision(DECISIONS.withdraw.value)}
            >
              <input
                type="checkbox"
                name="checkbox"
                className="checkbox checkbox-accent h-4 w-4"
                checked={selectedDecision === DECISIONS.withdraw.value}
              />
...
export const Footer = ({
  ...
  return (
    <div className="flex flex-row gap-3 my-6 justify-between px-6">
      <RetroButton type="button" onClick={() => prev()}>
        PREV
      </RetroButton>
      <div className="hidden md:block"></div>
      <RetroButton type="button" onClick={() => next()}>
        NEXT
      </RetroButton>
    </div>
  );
...
export default function Home() {
  ...
  const currentApplicationDecisionId = applicationDecisionId
    ? applicationDecisionId[0]
    : undefined;
  const { application } = useRetrieveRecord({ id: currentApplicationId });
  const { application: decisionRecord, refetchRetrieveRecord } =
    useRetrieveRecord({ id: currentApplicationDecisionId });
  const applicationDecision = decisionRecord?.fields
    .DECISION as Decision["value"];
  ...
  const nextApplicantIndex = () =>
    setApplicantIndex((curr) => {
  ...
  const parseForHyperlinks = (txt: string | undefined): ReactNode => {
    ...
    return txt.split(" ").map((part, key) =>
      URL_REGEX.test(part) ? (
        <a
          href={part.startsWith("https") ? part : `https://${part}`}
          key={key}
          className="underline"
          target="_blank"
        >
  ...
  if (!isSearcher) {
    return (
      <Main>
        <div className="text-4xl font-playfair p-6">You are not a Searcher</div>
      </Main>
    );
  }
